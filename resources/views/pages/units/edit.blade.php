...
@section('main')
<div class="mb-2">
    <h1 class="text-3xl font-bold text-gray-800 mb-2">Edit Unit</h1>
    <p class="text-gray-600">Update unit information</p>
</div>
    ...
    <form method="POST" action="{{ route('units.update', $unit) }}" class="space-y-6">
            ...
            <div>
                ...
                <select name="cluster_id" id="cluster_id"
                    ...
                    @foreach($clusters as $cluster)
                        <option value="{{ $cluster->id }}" data-site-plan="{{ $cluster->site_plan_path ? Storage::url($cluster->site_plan_path) : '' }}" {{ old('cluster_id', $unit->cluster_id) == $cluster->id ? 'selected' : '' }}>
                            {{ $cluster->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
                ...
                <input type="text" name="block" id="block" value="{{ old('block', $unit->block) }}"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
            ...
            <div>
                ...
                <input type="text" name="number" id="number" value="{{ old('number', $unit->number) }}"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
            ...
            <div>
                ...
                <input type="text" name="house_type" id="house_type" value="{{ old('house_type', $unit->house_type) }}"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="building_area" id="building_area" value="{{ old('building_area', $unit->building_area) }}" step="0.01"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="hidden" name="progress" id="progress" value="{{ old('progress', $unit->progress) }}">
                ...
                <div class="mt-3">
                    <div class="flex justify-between text-sm text-gray-600 mb-1">
                        <span>Progress</span>
                        <span id="progress-text">{{ $unit->progress }}%</span>
                    </div>
                    <div id="progress-container" class="relative w-full bg-gray-200 rounded-full h-6 cursor-pointer">
                        <div id="progress-bar" class="bg-[#2FA769] h-6 rounded-full transition-all duration-300 flex items-center justify-end" style="width: {{ $unit->progress }}%">
        ...
        <div id="site-plan-section" class="hidden space-y-4">
            ...
            <input type="hidden" name="coordinates" id="coordinates" value="{{ old('coordinates', $unit->coordinates) }}">
            ...
            <p class="text-sm text-gray-500">Click on the site plan to set the unit location. Current coordinates: <span id="coord-display">{{ $unit->coordinates ? $unit->coordinates : 'Not set' }}</span></p>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    clusterSelect.addEventListener('change', function() {
        ...
        if (sitePlanUrl) {
            ...
            // Wait for image to load before positioning point
            sitePlanImage.onload = function() {
                if (coordinatesInput.value) {
                    const [xPercent, yPercent] = coordinatesInput.value.split(',');
