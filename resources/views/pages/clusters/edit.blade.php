...
<div class="mb-2">
    ...
    <p class="text-gray-600">Update cluster information</p>
</div>
<div class="max-w-full mx-auto bg-white rounded-xl shadow-lg border border-gray-200 p-8">

    <form method="POST" action="{{ route('clusters.update', $cluster) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $cluster->name) }}"
            ...
            <div>
                ...
                <select name="developer_id" id="developer_id"
                    ...
                    @foreach(\App\Models\Company::active()->get() as $company)
                        <option value="{{ $company->id }}" {{ old('developer_id', $cluster->developer_id) == $company->id ? 'selected' : '' }}>
                            {{ $company->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="grid grid-cols-1 gap-6">
            <div>
                ...
                <textarea name="address" id="address" rows="3"
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
                          placeholder="Full address of the cluster" required>{{ old('address', $cluster->address) }}</textarea>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
                          placeholder="Detailed description of the cluster">{{ old('description', $cluster->description) }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="latitude" id="latitude" value="{{ old('latitude', $cluster->latitude) }}" step="any"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
                       placeholder="-6.2088">
            ...
            <div>
                ...
                <input type="number" name="longitude" id="longitude" value="{{ old('longitude', $cluster->longitude) }}" step="any"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
        ...
        <div class="grid grid-cols-1 gap-6">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Current Site Plan</label>
                <div class="mt-1">
                    <img src="{{ Storage::url($cluster->site_plan_path) }}" alt="Current Site Plan" class="max-w-xs max-h-48 object-cover rounded-lg border border-gray-300">
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="grid grid-cols-1 gap-6">
            <div>
                <label for="site_plan" class="block text-sm font-semibold text-gray-700 mb-2">{{ $cluster->site_plan_path ? 'Replace' : 'Upload' }} Site Plan / Denah</label>
                    ...
                    <div class="space-y-1 text-center">
                        ...
                        <div class="flex text-sm text-gray-600">
                            <label for="site_plan" class="relative cursor-pointer bg-white rounded-md font-medium text-[#2FA769] hover:text-[#256f4a] focus-within:outline-none">
                                <span>{{ $cluster->site_plan_path ? 'Replace' : 'Upload' }} site plan</span>
                ...
                <div id="site_plan_preview" class="mt-4 hidden">
                    <p class="text-sm font-semibold text-gray-700 mb-2">New Preview:</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                ...
                <input type="text" name="area_size" id="area_size" value="{{ old('area_size', $cluster->area_size) }}"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
                       placeholder="5000 m²">
            ...
            <div>
                ...
                <input type="number" name="total_units" id="total_units" value="{{ old('total_units', $cluster->total_units) }}"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
                       min="0">
            ...
            <div>
                ...
                <input type="number" name="available_units" id="available_units" value="{{ old('available_units', $cluster->available_units) }}"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
                       min="0">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="price_range_min" id="price_range_min" value="{{ old('price_range_min', $cluster->price_range_min) }}"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
                       min="0" step="1000000">
            ...
            <div>
                ...
                <input type="number" name="price_range_max" id="price_range_max" value="{{ old('price_range_max', $cluster->price_range_max) }}"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
                       min="0" step="1000000">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="year_built" id="year_built" value="{{ old('year_built', $cluster->year_built) }}"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2FA769] focus:border-[#2FA769] transition duration-200 bg-gray-50 focus:bg-white"
            ...
            <div>
                ...
                <div class="space-y-2">
                    @php
                        ...
                        $oldFacilities = old('facilities', $cluster->facilities ?? []);
                    @endphp
        ...
        <div class="flex items-center space-x-3">
            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $cluster->is_active) ? 'checked' : '' }}
