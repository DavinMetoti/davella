...
@section('main')
<div class="mb-2">
    <h1 class="text-2xl font-bold text-gray-800 mb-2">Edit Menu</h1>
    ...
    <form method="POST" action="{{ route('menus.update', $menu) }}" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="hidden" name="icon" id="icon" value="{{ old('icon', $menu->icon) }}">
                <div class="mt-2 flex items-center space-x-2">
                    <div id="icon-preview" class="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-lg">
                        <i id="preview-icon" class="{{ old('icon', $menu->icon) }} text-gray-600"></i>
                    </div>
                    <span id="icon-text" class="text-sm text-gray-600">{{ old('icon', $menu->icon) }}</span>
                </div>
